...
from .get_data import get_snapshot
...
def get_filtered_data(db_path: str, table_names: list, output_table_name: str):
    ...
    try:
        ...
        for table_name in table_names:
            ...
            # 只保留需要的列
            final_df = (
                filtered_df[["策略名称", "证券代码"]]
                .drop_duplicates()
                .merge(grouped, on="证券代码")
            )
...
def process_and_merge_data(db_path: str, table_name: str, xt_trader: str, acc: str):
    ...
    try:
        ...
        positions_data = pd.DataFrame(
            [{"证券代码": pos.stock_code, "持仓量": pos.volume} for pos in positions]
        )
...
def mole_hunting_delegation(
    ...
    try:
        # 执行函数并获得DataFrame
        df = process_and_merge_data(db_path, table_name, xt_trader, acc)
        ...
        # 判断是否触发止盈条件
        sell_cond = (
            df["max_pct_chg"] - df["pct_chg"]
        ) > drawdown  # 最大涨幅减去今日涨幅大于最大回撤跌幅,即触发上涨止盈后回撤多少止盈
        sell_cond &= df["max_pct_chg"] > active_thres  # 最大涨幅>多少点后，触发移动止盈
        sell_cond &= df["pct_chg"] > deal_thres  # 今日涨幅>多少点后，才可以止盈
        sell_cond &= (
            df["持仓数量"] == df["持仓量"]
        )  # 假设只有满仓时才考虑止盈，持仓数量是计划持仓数量，持仓量是实际持仓数量
        sell_df = df.loc[sell_cond].copy()

        # 判断是否触发买入条件（这里需要您定义买入条件）
        buy_cond = (df["max_pct_chg"] - df["pct_chg"]) > (
            drawdown + 1.0
        )  # 最大涨幅减去今日涨幅大于最大回撤跌幅再加1%，以此来确保卖出后买入有1%的收益率
        buy_cond &= (
            df["max_pct_chg"] > active_thres
        )  # 最大涨幅>多少点后，触发移动止盈，只有触发移动止盈，即有了打地鼠之后再会有买回的考虑
        buy_cond &= (df["high"] + df["low"] + df["pre_close"]) / 3 > df[
            "close"
        ]  # （最高价+最低价+昨收价）的平均价大于现价，即买入的价格在平均价上下
        buy_cond &= df["持仓数量"] > df["持仓量"]  # 假设无持仓时才考虑买入
        buy_df = df.loc[buy_cond].copy()
        ...
        if not sell_df.empty:
            ...
            for _index, row in sell_df.iterrows():
        ...
        if not buy_df.empty:
            ...
            buy_df["remark"] = "动态买入单"
            for _index, row in buy_df.iterrows():
                xt_trader.order_stock(
                    acc,
                    row["证券代码"],
        ...
        try:
            ...
            if asset.cash >= 1000:
                ...
                xtdata.subscribe_quote(
                    "131810.SZ", period="tick", start_time="", end_time="", count=1, callback=None
                )
                xtdata.subscribe_quote(
                    "204001.SH", period="tick", start_time="", end_time="", count=1, callback=None
                )
                generate_func = xtdata.get_market_data(
                    field_list=["bidPrice"],
                    stock_list=["131810.SZ", "204001.SH"],
                    period="tick",
                    start_time="",
                    end_time="",
                    count=-1,
                ...
                print(
                    f"成功下达订单，股票代码：{stock_code}，价格：{price}，订单量：{order_volume}。"
                )
            else:
                print("可用资金不足，不进行交易")
        except Exception as e:
