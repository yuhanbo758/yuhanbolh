...
def convert_time(unix_timestamp):
    traded_time_utc = datetime.utcfromtimestamp(
        unix_timestamp
    )  # 注意这里改为 datetime.utcfromtimestamp
...
def calculate_remaining_holdings(db_path, strategy_tables):
    try:
        ...
        for table in strategy_tables:
            strategy_data = pd.read_sql(
                f"SELECT 证券代码, SUM(成交数量 * 买卖) as total FROM {table} GROUP BY 证券代码",
                conn,
            ).set_index("证券代码")
            strategy_data["total"] = strategy_data["total"].astype(int)
            account_holdings["持仓数量"] -= (
                account_holdings.join(strategy_data, how="left")["total"].fillna(0).astype(int)
            )
        ...
        if connect_result == 0:
            ...
            for _i, row in frame.iterrows():
                ...
                if order_type == xtconstant.STOCK_BUY:
                    ...
                    # 针对特定表名增加额外的现金保留条件
                    if (
                        table_name in ["place_fund_grid_order", "place_fund_basics_technical_order"]
                        and asset.cash <= 3000
                    ):
...
class MyXtQuantTraderCallback(XtQuantTraderCallback):
    ...
    def on_account_status(self, status):
        ...
        """
        print(
            '账户状态，类型2为"证券账户"，状态：-1-无效；0-正常；4-初始化'
        )  # http://docs.thinktrader.net/vip/pages/198696/#%E8%B4%A6%E5%8F%B7%E7%8A%B6%E6%80%81-account-status
            ...
            if "09:00" <= current_time <= "16:10" and 0 <= current_weekday <= 4:
                ...
                if connect_result != 0:
                    ...
                    if subscribe_result != 0:
                        print(f"账号订阅失败 {subscribe_result:d}")
                    print("连接成功")
                    asset = xt_trader.query_stock_asset(acc)
                    if asset:
                        save_stock_asset(asset, db_path)
