...
def get_tdx_market_address(cfg_file):
    try:
        if os.path.exists(cfg_file):
            ...
            with open(cfg_file, encoding="gb2312") as f:
                config.read_file(f)
...
def get_security_quotes(market_codes):
    ...
    return get_financial_data(
        global_tdx_ip, global_tdx_port, lambda api: api.get_security_quotes(market_codes)
    )
...
def get_security_bars(category, market, code, start, count):
    ...
    return get_financial_data(
        global_tdx_ip,
        global_tdx_port,
        lambda api: api.get_security_bars(category, market, code, start, count),
    )
...
def get_security_count(market):
    ...
    """
    return get_financial_data(
        global_tdx_ip, global_tdx_port, lambda api: api.get_security_count(market)
    )
...
def get_index_bars(category, market, code, start, count):
    ...
    return get_financial_data(
        global_tdx_ip,
        global_tdx_port,
        lambda api: api.get_index_bars(category, market, code, start, count),
    )
...
def get_history_minute_time_data(market, code, date):
    ...
    return get_financial_data(
        global_tdx_ip,
        global_tdx_port,
        lambda api: api.get_history_minute_time_data(market, code, date),
    )
...
def get_transaction_data(market, code, start, count):
    ...
    return get_financial_data(
        global_tdx_ip,
        global_tdx_port,
        lambda api: api.get_transaction_data(market, code, start, count),
    )
...
def get_finance_info(market, code):
    ...
    """
    return get_financial_data(
        global_tdx_ip, global_tdx_port, lambda api: api.get_finance_info(market, code)
    )
...
def qmt_data_source(stock_code, days=7 * 365):
    ...
    # 将时间戳转换为日期字符串
    data_transposed["time"] = pd.to_datetime(data_transposed["time"], unit="ms") + pd.Timedelta(
        hours=8
    )  # 加上时区偏移
...
def qmt_data_source_download(stock_code, days=7 * 365):
    ...
    xtdata.download_history_data2(
        [stock_code], period="1d", start_time=start_time, callback=on_progress
    )
...
def download_7_years_data(stock_list):
    ...
    # 下载近7年的历史数据
    xtdata.download_history_data2(
        stock_list, period="1d", start_time=start_time, callback=on_progress
    )
...
def filter_bond_cb_redeem_data_and_save_to_db():
    try:
        ...
        filtered_rows = filtered_df[
            filtered_df["强赎天计数"].str.contains("已公告强赎|公告要强赎")
        ].copy()

        # 修改“可转债代码”列的数据
        filtered_rows.loc[filtered_rows["可转债代码"].str.startswith("11"), "可转债代码"] = (
            filtered_rows["可转债代码"] + ".SH"
        )
        filtered_rows.loc[filtered_rows["可转债代码"].str.startswith("12"), "可转债代码"] = (
            filtered_rows["可转债代码"] + ".SZ"
        )
        ...
        for table_name in table_names:
            ...
            for strategy_name in strategies:
                ...
                for _, row in strategy_df.iterrows():
                    if row["买卖"] == 1:  # 买入
                        unhedged_transactions = pd.concat(
                            [unhedged_transactions, pd.DataFrame([row])], ignore_index=True
                        )
                    elif row["买卖"] == -1:  # 卖出
                ...
                # 筛选出未完全对冲的买入交易
                remaining_buys = unhedged_transactions[
                    (unhedged_transactions["成交数量"] > 0) & (unhedged_transactions["买卖"] == 1)
                ]

                # 累积所有策略的未对冲买入交易
                all_remaining_buys = pd.concat(
                    [all_remaining_buys, remaining_buys], ignore_index=True
                )
...
def position_close_process_data(
    table_name, db_path=r"D:\wenjian\python\smart\data\guojin_account.db"
):
...
def get_snapshot(code_list: list[str]):
    ...
    df.loc[(df["bid1"] == 0) | (df["ask1"] == 0), "averagePrice"] = (
        df["bid1"] + df["ask1"]
    )  # 涨跌停修正

    df.rename(
        columns={"averagePrice": "close", "lastClose": "pre_close", "volume": "vol"}, inplace=True
    )
