...
def get_valuation_ratios(code):
    try:
        ...
        if response.status_code == 200:
            ...
            # 根据code的前两位决定匹配模式
            if code[:2] == "HK":
                pattern_master_value = re.compile(
                    r"大师价值 : HK\$ [\d\.]+ \(今日\)[\s\S]*?HK\$ (\d+\.\d+)"
                )
            else:
            ...
            if match:
                ...
                data_pattern = re.compile(rf"[¥$]{element_xj}\s+(.*?)\s+(.*?)\s+(.*?)\s+(.*?)\s+")
                ...
                pj_pattern = re.compile(
                    rf"[¥$]{element_xj}\s+(.*?)\s+(.*?)\s+(.*?)\s+(.*?)\s+(.*?)\s+(.*?)\s+"
                )
...
def ichimoku_cloud_zb(data, conversion_periods, base_periods, lagging_span2_periods, displacement):
    ...
    data["close"].shift(-displacement + 1).shift(25).ffill()
    lead_line1.shift(displacement - 1)
    lead_line2.shift(displacement - 1)
    # 计算买入和卖出信号
    buy_signal = (
        (base_line < data["close"])
        & (conversion_line > data["close"].shift(1))
        & (conversion_line <= data["close"])
        & (lead_line1 > data["close"])
...
def ADX_zb(data, n):
    ...
    plus = (
        100
        * pd.Series(plusDM).ewm(alpha=1 / n, min_periods=n).mean()
        / pd.Series(truerange).ewm(alpha=1 / n, min_periods=n).mean()
    )
    minus = (
        100
        * pd.Series(minusDM).ewm(alpha=1 / n, min_periods=n).mean()
        / pd.Series(truerange).ewm(alpha=1 / n, min_periods=n).mean()
    )
...
def AO_zb(data):
    ...
    # 根据条件计算买入、卖出和中立的信号
    signal = np.where(
        AO_plus_saucer | AO_plus_cross, 1, np.where(AO_minus_saucer | AO_minus_cross, -1, 0)
    )
...
def WPR_zb(data, n):
    ...
    # 否则，信号为0（中立）
    signal = np.where(
        (WPR < lower_band) & (WPR > WPR.shift(1)),
        1,
...
def BBP_zb(data, n):
    ...
    trend = np.where(data["close"] > moving_avg, 1, np.where(data["close"] < moving_avg, -1, 0))

    # 根据牛市力量、熊市力量和股价趋势来决定买卖信号
    signal = np.where(
        (trend == 1) & (bearPower < 0) & (bearPower > bearPower.shift(1)),
        1,
...
def generate_stat_data_zb(stock_code):
    ...
    # 判断条件并输出结果
    stat_data["技术方向"] = np.where(
        (mean_value >= -1.0) & (mean_value < -0.5),
        -2,
        np.where(
            (mean_value >= -0.5) & (mean_value < -0.1),
            -1,
            np.where(
                (mean_value >= -0.1) & (mean_value <= 0.1),
                0,
                np.where(
                    (mean_value > 0.1) & (mean_value <= 0.5),
                    1,
    ...
    stat_data["signal_lr7"] = np.where(
        stat_data["close"] < (stat_data["expected_value_7year"] - stat_data["std_residuals_7year"]),
        1,
        np.where(
            stat_data["close"]
            > (stat_data["expected_value_7year"] + stat_data["std_residuals_7year"]),
            -1,
            0,
        ),
    )
    stat_data["signal_lr3"] = np.where(
        stat_data["close"] < (stat_data["expected_value_3year"] - stat_data["std_residuals_3year"]),
        1,
        np.where(
            stat_data["close"]
            > (stat_data["expected_value_3year"] + stat_data["std_residuals_3year"]),
            -1,
            0,
        ),
    )
    stat_data["signal_lr1"] = np.where(
        stat_data["close"]
        < (stat_data["expected_value_1year"] - 2 * stat_data["std_residuals_1year"]),
        1,
        np.where(
            stat_data["close"]
            > (stat_data["expected_value_1year"] + 2 * stat_data["std_residuals_1year"]),
    ...
    stat_data["lr_sum"] = (
        stat_data["signal_lr7"] + stat_data["signal_lr3"] + stat_data["signal_lr1"]
    )
    stat_data["线性回归"] = np.where(
        stat_data["lr_sum"] <= -2,
        -2,
        np.where(
            stat_data["lr_sum"] == -1,
            ...
            np.where(
                stat_data["lr_sum"] == 0,
                0,
                np.where(stat_data["lr_sum"] == 1, 1, np.where(stat_data["lr_sum"] >= 2, 2, 0)),
            ),
        ),
    )
...
def ex_fund_valuation(db_path, table_name_guojin, table_name_result):
    ...
    for code in guojin_data["mt5代码"]:
        try:
            ...
            # 合并行数据与估值比率数据
            ratios_new = pd.concat(
                [row_data_df.reset_index(drop=True), ratios.reset_index(drop=True)], axis=1
            )
...
def calculate_totals(db_path, ea_id, magic):
    ...
    # 打印所有列的总和
    print(
        f"总和: {total_commission + total_swap_forex_trades + total_profit_forex_trades + total_swap_position + total_profit_position}"
    )
...
if __name__ == "__main__":
    ...
    if not mt5.initialize(
        path=r"D:\jiaoyi\IC-MT5-Demo\terminal64.exe",
        login=51455171,
        password="LiEbcs6r",
