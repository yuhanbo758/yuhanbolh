...
def export_positions_to_db(db_path, table_name):
    try:
        ...
        if positions is None:
            ...
            df_positions["time"] = (
                pd.to_datetime(df_positions["time"], unit="s")
                .dt.tz_localize("UTC")
                .dt.tz_convert(tz_beijing)
            )
...
def insert_into_db(db_path, result):
    try:
        ...
        cursor.execute(
            "INSERT INTO forex_trades (交易类型, EA_id, 订单号, 品种名称, 交易量, 价格, Limit挂单, 止损, 止盈, 价格偏差, 订单类型, 成交类型, 订单有效期, 订单到期, 订单注释, 持仓单号, 反向持仓单号) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            (
                request["action"],
                request["magic"],
                request["order"],
...
def execute_order_from_db(db_path, table_name):
    ...
    # 遍历DataFrame中的每一行
    for _index, order_info in df.iterrows():
        try:
            ...
            existing_orders = mt5.orders_get(
                symbol=symbol, magic=int(order_info["EA_id"]), type=order_type
            )
            if existing_orders:
...
def process_non_strategy_positions(db_path):
    ...
    for _index, position in df_non_strategy_positions.iterrows():
        # 从数据表中提取相应的值
...
def schedule_jobs():
    ...
    hours = list(range(7, 24)) + list(range(4))  # 周一到周五全天，周六到凌晨4点
    minutes = ["05", "35"]
    [f"{hour:02d}:{minute}" for hour in hours for minute in minutes]
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if 0 <= current_time.weekday() < 6 and (
            7 <= current_time.hour < 24 or current_time.weekday() == 5 and current_time.hour < 4
        ):
            # 初始化MetaTrader 5连接
