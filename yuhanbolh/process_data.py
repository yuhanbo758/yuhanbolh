...
def get_valuation_ratios(code):
    try:
        ...
        # 获取大师价值估值评价
        element_pj = soup.select_one(
            "#q-app > div > div > main > div:nth-child(3) > div > div.column.term-container.col > div.content-section.q-pa-md.q-mt-sm.q-card.q-card--bordered.q-card--flat.no-shadow > div > div:nth-child(9) > span"
        ).text.strip()
        ...
        # 获取名称
        element_mc = soup.select_one(
            "html body div div main div:nth-child(1) div:nth-child(2) div:nth-child(1) div:nth-child(1) div:nth-child(2) div:nth-child(1) h1 span:nth-child(1)"
        ).text.strip()

        # 获取现价
        element_xj = soup.select_one(
            "html body div div main div:nth-child(1) div:nth-child(2) div:nth-child(1) div:nth-child(1) div:nth-child(2) div:nth-child(2)"
        )
        element_xj.span.decompose()  # 去掉span标签
        element_xj = re.findall(r"\d+\.\d+", element_xj.text.strip())[0]  # 只保留数字和小数点

        # 获取市净率、市盈率、股息率和总市值的基本数据
        element_data = soup.select_one(
            "#q-app > div > div > main > div.q-pt-sm.bg-white.shadow-1 > div.row.page-width-1440.paywall-trigger.login-trigger > div.col-12.col-md.q-mb-xs.q-mx-xs.q-card.q-card--flat.no-shadow.q-px-md > div:nth-child(2) > div"
        ).text.strip()
...
def VWMA(data, n):
    VWMA = pd.Series(
        (data["close"] * data["volume"]).rolling(n).sum() / data["volume"].rolling(n).sum(),
        name="VWMA_" + str(n),
    )
    return VWMA.dropna()
...
def STOK(data, n, m, t):
    ...
    result = pd.DataFrame(
        {"Stochastic_%K": k, "Stochastic_%D": d, "Stochastic_%D_signal": d_signal}, index=data.index
    )  # 使用原始数据的索引
...
def ADX(data, n):
    ...
    # 显式继承输入索引，避免 pandas 3 的严格索引对齐把日期索引结果变成 NaN。
    plusDM = pd.Series(np.where((up > down) & (up > 0), up, 0), index=data.index)
    minusDM = pd.Series(np.where((down > up) & (down > 0), down, 0), index=data.index)
    ...
    plus = (
        100
        * plusDM.ewm(alpha=1 / n, min_periods=n).mean()
        / truerange.ewm(alpha=1 / n, min_periods=n).mean()
    )
    minus = (
        100
        * minusDM.ewm(alpha=1 / n, min_periods=n).mean()
        / truerange.ewm(alpha=1 / n, min_periods=n).mean()
    )
    sum = plus + minus
    adx = (
        100
        * (np.abs(plus - minus) / np.where(sum == 0, 1, sum)).ewm(alpha=1 / n, min_periods=n).mean()
    )
...
def AO(data):
    AO = (data["high"].rolling(5).mean() + data["low"].rolling(5).mean()) / 2 - (
        data["high"].rolling(34).mean() + data["low"].rolling(34).mean()
    ) / 2
...
def MACD_Level(data, n_fast, n_slow):
    ...
    # 创建一个新的DataFrame来存储结果
    result = pd.DataFrame(
        {"MACD": MACD, "MACDsignal": MACDsignal, "MACDhist": MACDhist}, index=data.index
    )  # 使用原始数据的索引
...
def Stoch_RSI(data, smoothK, smoothD, lengthRSI, lengthStoch):
    ...
    rsi = 100 - 100 / (1 + rs)
    # 计算Stochastic
    stoch = (rsi - rsi.rolling(window=lengthStoch).min()) / (
        rsi.rolling(window=lengthStoch).max() - rsi.rolling(window=lengthStoch).min()
    )
    ...
    # 创建一个新的DataFrame来存储结果
    result = pd.DataFrame(
        {"Stoch_RSI_K": k, "Stoch_RSI_D": d}, index=data.index
    )  # 使用原始数据的索引
...
def WPR(data, n):
    WPR = pd.Series(
        (data["high"].rolling(n).max() - data["close"])
...
def linear_regression_dfcf(data, days_list):
    ...
    for many_days in days_list:
        ...
        expected_values = (
            ...
            .apply(
                lambda y: (
                    stats.linregress(np.arange(len(y)), y)[1]
                    + stats.linregress(np.arange(len(y)), y)[0] * (len(y) - 1)
                ),
                raw=True,
            )
        )
        ...
        std_residuals = (
            ...
            .apply(
                lambda y: np.std(
                    ...
                    - (
                        stats.linregress(np.arange(len(y)), y)[1]
                        + stats.linregress(np.arange(len(y)), y)[0] * np.arange(len(y))
                    )
                ),
                raw=True,
            )
...
def calculate_annual_return(table_name):
    ...
    periods = (
        data["成交时间"] - data["成交时间"].min()
    ).dt.days / 365.25  # 计算每笔现金流的时间（以年为单位）
    npv = sum(data["现金流"] / (1 + discount_rate) ** periods)

    # 创建一个新的DataFrame来返回结果
    result = pd.DataFrame(
        {"年化收益率": [annual_return], "现金流之和": [cash_flows_sum], "净现值": [npv]}
    )
...
def get_processed_code(stock_code):
    ...
    if stock_code.startswith("6"):  # 沪市股票
        ...
        stock_code.startswith("50") or stock_code.startswith("51") or stock_code.startswith("58")
    ):  # 沪市场内基金
        market = "SH"
    elif (
        stock_code.startswith("15") or stock_code.startswith("16") or stock_code.startswith("18")
    ):  # 深市场内基金
        market = "SZ"
    else:
        return "轮动定时"
...
def clean_execute_general_trade(conn):
    ...
    cursor.execute(
        "SELECT rowid, 策略名称, 证券代码 FROM execute_general_trade WHERE 策略名称 != '问财轮动-注询问'"
    )
...
def delete_execute_general_trade_row(cursor, conn, strategy, code):
    ...
    """
    cursor.execute(
        "DELETE FROM execute_general_trade WHERE 策略名称=? AND 证券代码=?", (strategy, code)
    )
    conn.commit()
...
def process_price_grid(
    cursor, conn, tick, code, price, quantity, buy_sell, strategy, remark, datetime_str, rowid
):
    """
    处理价差网格策略
    """
    ...
    if not trade_rows:
        if (price - 委托备注_value) >= lastPrice:
            ...
            insert_order(
                cursor, conn, code, new_price, 委托数量_value, 1, strategy, remark, datetime_str
            )
    else:
    ...
    try:
        ...
        if not trade_rows:
            ...
            if buy_price >= lastPrice:
                insert_order(
                    cursor, conn, code, buy_price, 委托数量_value, 1, strategy, remark, datetime_str
                )
...
def process_immediate_rows(rows, cursor, conn):
    # xtquant 无法通过普通 PyPI 安装；仅在执行 QMT 行情逻辑时按需导入，
    # 使纯 NumPy/pandas 指标用户无需安装交易终端也能使用本模块。
    ...
    for row in rows:
        ...
        if datetime_str:
            ...
            if current_time > row_time:
                ...
                cursor.execute(
                    "DELETE FROM execute_general_trade WHERE 策略名称=? AND 证券代码=?",
                    (strategy, code),
                )
                conn.commit()
                continue
        ...
        if strategy == "即时委托":
            insert_order(
                cursor, conn, code, price, quantity, buy_sell, strategy, remark, datetime_str
            )
            cursor.execute("DELETE FROM receive_condition WHERE rowid=?", (rowid,))
            conn.commit()
            # 删除对应的execute_general_trade行
            ...
            try:
                if lastClose != 0:
                    ...
                    if (price_change_ratio >= price / 100) and (
                        retreat_ratio >= float(remark) / 100
                    ):
                        insert_order(
                            cursor,
                            conn,
...
def portfolio_rotation(order_note, order_quantity, strategy_name):
    ...
    try:
        ...
        securities_codes = securities_codes.assign(
            委托备注=order_note, 委托数量=order_quantity, 策略名称=strategy_name
        )
    ...
    try:
        for row in scheduled_tasks:
            ...
            for buy_code in securities_to_buy:
                A_data[A_data["证券代码"] == buy_code].iloc[0]
                insert_order(
                    cursor,
                    conn,
                    buy_code,
